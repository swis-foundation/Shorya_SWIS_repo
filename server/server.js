    ...
    if (event.event === 'payment.captured') {
        ...
        try {
            ...
            const donationQuery = `
                ...
                razorpay_payment_id = EXCLUDED.razorpay_payment_id, status = 'success', is_anonymous = EXCLUDED.is_anonymous, donor_email = EXCLUDED.donor_email;
            `;
            ...
            if (campaignResult.rows.length > 0 && isNewSuccess) {
...
app.post('/api/payments/verify-payment', async (req, res) => {
    ...
    if (expectedSignature === razorpay_signature) {
        ...
        try {
            await client.query('BEGIN');
            const existingDonation = await client.query('SELECT status FROM donations WHERE razorpay_order_id = $1 FOR UPDATE', [razorpay_order_id]);
            const isNewSuccess = !existingDonation.rows.length || existingDonation.rows[0].status !== 'success';
